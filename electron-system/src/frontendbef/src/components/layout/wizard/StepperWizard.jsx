		...
		<div className="w-full px-24 py-4">
			<Stepper activeStep={activeStep} isLastStep={(value) => setIsLastStep(value)} isFirstStep={(value) => setIsFirstStep(value)}>
				<Step onClick={() => setActiveStep(0)}>
					<UserIcon className="h-7 w-7 p-1" />
				...
				<Step onClick={() => setActiveStep(2)}>
					<BuildingLibraryIcon className="h-7 w-7 p-1" />
					<div className="absolute -bottom-[4.5rem] w-max text-center">
			...
			<div className="mt-32 flex justify-between">
				<Button variant="content" onClick={handlePrev} disabled={isFirstStep}>
					Prev
				</Button>
				<Button variant="content" onClick={handleNext} disabled={isLastStep}>
					Next
				</Button>
			</div>
		</div>
