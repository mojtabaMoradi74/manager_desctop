...
const Step3 = ({ config, updateConfig, nextStep, prevStep }) => {
	const [dbType, setDbType] = useState(config.dbType || "postgres");
		...
		<div className="  mx-auto p-6 bg-white rounded-lg shadow">
			...
			<div className="space-y-4 mb-6">
				{/* <Radio
					...
					color="blue"
				/> */}
				<label>{"PostgreSQL (پیشنهادی)"}</label>
				...
				<div className="mb-6 p-4 border rounded-lg">
					...
					{dbStatus?.installed && !dbStatus.running && (
						<div>
							...
							<Button color="red" onClick={installDatabase} loading={isChecking} className="mt-2">
								راه‌اندازی {dbType === "postgres" ? "PostgreSQL" : "SQL Server"}
							</Button>
						</div>
					)}
				</div>
