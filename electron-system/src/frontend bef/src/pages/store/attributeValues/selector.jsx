...
const convertor = {
  object: (x) =>
    x && {
      ...
      image: x?.image,
      order: x?.order,
      usedForVariation: x?.usedForVariation || x?.associations?.usedForVariation,
      showInProduct: x?.showInProduct || x?.associations?.showInProduct,
      data: x,
    },
...
const Element = ({queryParams, ...props}) => {
  ...
  const resQuery = useQueryCustom({
    name: `${Enum?.api?.base}`,
    url: getApi,
    params: {...apiParams, ...props.apiParams},
  })
  ...
  // ----------------------------------------------------------------
  console.log('* * * attributeValuesSelector onSuccess : ', {resQuery, options})
...
const attributeValuesSelector = {
  Element,
  convertor,
}

export default attributeValuesSelector
