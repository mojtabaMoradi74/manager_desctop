...
const Add = ({onClose}) => {
  ...
  const backUrl = `${Enum.routes.root(parent)}`
  const methods = useForm({
    resolver: yupResolver(validation.schema(paramId)),
    defaultValues: {
      [validation.fieldNames.color]: '#000',
    },
  })
  ...
  const parentAttributeData = parentDataById?.data?.data
            ...
            <FormItem label={t('basic info')}>
              ...
              >
                <Grid container spacing={5}>
                  ...
                  <Grid item xs={12} md={12}>
                    {parentAttributeData?.type == EAttributes.types.object.color.value ? (
                      <RHFTextField
                        required
                        {...{
