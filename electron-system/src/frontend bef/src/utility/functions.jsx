...
export const CryptoIcon = (name) => {
  ...
  if (nameSplited.length === 2) {
    return 'https://cryptoicon-api.vercel.app/api/icon/' + nameSplited[0].toLowerCase()
  } else {
...
export function dateFormatter(dateutc) {
  const val = new Date(dateutc ? dateutc : null).toISOString().split('.')[0] + 'Z'
...
export function truncateString(str, num) {
  ...
  if (num > str.length) {
    ...
    // or if the num is less than 3 (11 is not less than 3)
    str.slice(3)
    return str.append('...')
  } else {
    // do if no if was matched (and here we are)
    return 'This is not a string'
  }
}

export const getImageUrl = (image, defaultFallback = '/media/no-image.png', isAvatar = false) => {
  // console.log({ image }, 'this is image');
  if (image?.[0] && image?.[0]?.key && !image?.[0]?.location) {
    return image?.[0]?.key
    ...
    return image
  } else if (image?.[0] && image?.[0]?.location) {
    return image?.[0]?.location
  } else {
    if (isAvatar) {
