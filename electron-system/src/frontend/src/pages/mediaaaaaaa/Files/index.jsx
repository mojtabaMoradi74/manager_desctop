    ...
    <div className='border-top pt-3'>
      <div className='flex items-center justify-between mx-0 '>
        <div className='col-sm-6 p-md-0'>{` ${
          folder?.title ? `فایل های پوشه ${folder?.title}` : 'فایل ها'
        } (${data?.result?.length || 0})`}</div>
        <div className='gap-3 col-sm-6 p-md-0 justify-end mt-2 mt-sm-0 flex'>
      ...
      <div>
        {changeFolderFiles?.length ? (
          <div className='flex items-center gap-2'>
            <div>{`${changeFolderFiles?.length} files peraped to change folder`}</div>
            <Button
              color='success'
              variant='outlined'
