...
const Add = ({onClose}) => {
  ...
  const onSuccess = (response) => {
    ...
    const resetData = {
      ...params,
      [validation.fieldNames.category]: categorySelector.convertor.array(params.category),
      [validation.fieldNames.product]: productSelector.convertor.array(params.product),
      [validation.fieldNames.image]: params?.image,
      [validation.fieldNames.status]: StatusData[params.status],
      [validation.fieldNames.type]: Enum.type.object[params.type],
      [validation.fieldNames.linkType]: Enum.linkType.object[params.linkType],
    }
  ...
  const onSubmit = async () => {
    ...
    const reqData = {
      ...
      [validation.fieldNames.linkType]: values[validation.fieldNames.linkType]?.value,
      [validation.fieldNames.category]: values[validation.fieldNames.category]?.value,
      [validation.fieldNames.product]: values[validation.fieldNames.product]?.value,
    }
            ...
            <FormItem label={t('basic info')}>
              ...
              >
                <Grid container spacing={5}>
                  ...
                  <Grid item xs={12} md={12}>
                    <RHFTextField required name={validation.fieldNames.link} label={'link'} />
                  </Grid>

                  <Grid item xs={12} md={12}>
                    <RHFTextField required name={validation.fieldNames.order} label={'order'} />
                  </Grid>
                ...
                <Grid container spacing={4}>
                  <Grid item xs={12} md={12}>
                    <categorySelector.Element
                      name={validation.fieldNames.category}
                      label={'category'}
                      multiple
                    />
                  </Grid>
                  <Grid item xs={12} md={12}>
                    <productSelector.Element
                      name={validation.fieldNames.product}
                      label={'product'}
                      multiple
                    />
                  </Grid>
                  ...
                  <Grid item xs={12} md={12}>
                    <RHFSelector
                      required
                      options={Enum.linkType.array}
                      name={validation.fieldNames.linkType}
                      label={'linkType'}
                    />
                  </Grid>
                </Grid>
