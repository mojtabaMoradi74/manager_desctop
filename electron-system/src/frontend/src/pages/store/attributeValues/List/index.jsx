...
import EAttributes from '../../attribute/enum'
...
const List = ({access}) => {
  ...
  const parent = queryParams.parent
  ...
  const getApi = async () =>
    axiosInstance().get(`${Enum.api.base}`, {
      params: {...queryString, parent},
    })

  const deleteApi = ({id}) => axiosInstance().delete(`${Enum.api.base}/${id}`)
  const changeStatus = (params) => axiosInstance().put(Enum.api.changeStatus, params)
  // ---------------------------------------------------------------------
  const getByIdParentData = () => axiosInstance().get(EAttributes.api.base + '/' + parent)
  // ----------------------------------------------------------------------------- Query By ID Parent Attribute
  const onSuccessParentDataById = ({data}) => {
    console.log('* * * parent attribute', {data})
  }
  const parentDataById = useQueryCustom({
    name: `getById_${EAttributes.api.base}_${parent}`,
    url: getByIdParentData,
    onSuccess: onSuccessParentDataById,
    enabled: !!parent,
  })
  // ------------------------------------------------------- query Get
  const {
  ...
  const tableOptions = useMemo(() => {
    return tableData({
      baseRoute: `${Enum.routes.root(parent)}`,
      supplementIds: [parent],
      onClick: clickAction,
      active: {
  ...
  console.log({parentDataById, data})

  return (
    <Page title={t(`${title}`)}>
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <HeaderBreadcrumbs
          heading={`${title} (${parentDataById?.data?.data?.title})`}
          action={
            checkAccess?.create ? (
              <>
                <Button
