...
function SortableList({
  ...
  console.log({dragging, willEndDragging})
  ...
  const onDragEnd = useCallback(() => {
    ...
    console.log({draggedItemPosition})

    setTimeout(() => {
      setWillEndDragging(false)
      itemsOrder.current = newItemsOrder
      // if (Math.abs(draggedItemPosition?.shiftY) < 100) return;
      if (draggedItemPosition.current.previous === draggedItemPosition.current.new) return
  ...
  const onMouseUp = useCallback(
    (event) => {
      ...
      // if (+dragging.dragStartY === +event.pageY) return
      // console.log({ startDrag: dragging.dragStartY, moveDrag: event.pageY });
  ...
  useEffect(() => {
    if (willEndDragging) {
      ...
      list.current.childNodes[
        draggedItemPosition.current.previous
      ].style.transform = `translateY(${draggedItemPosition.current.shiftY}px)`
    }
  }, [willEndDragging])
