...
const TABLE_HEAD = [
  {id: 'name', label: 'اسم محصول', align: 'left'},
...
export default function EcommerceProductList() {
  ...
  const handleGetAllProducts = () => {
    ...
    getAllProducts(page + 1, 10)
      .then(({data}) => {
        setTableData({loading: false, data})
        setIsNotFound(data?.content?.length === 0)
      })
      .catch((err) => {
        setTableData({data: {content: [], totalElements: 0}, loading: false})
        console.log(err)
      })
  }
        ...
        <Card>
            ...
            <TableContainer sx={{minWidth: 800, mt: 1}}>
              ...
              <Table size={dense ? 'small' : 'medium'}>
                <TableHeadCustom
                  ...
                  rowCount={tableData.data?.content?.length}
                  // numSelected={selected.length}
                  // onSort={onSort}
                  // onSelectAllRows={(checked) =>
                  //   onSelectAllRows(
                ...
                <TableBody>
                  {(tableData.loading ? [...Array(rowsPerPage)] : tableData.data?.content)
          ...
          <Box sx={{position: 'relative'}}>
            <TablePagination
              ...
              component='div'
              count={tableData.data?.totalElements}
              rowsPerPage={10}
              page={page}
