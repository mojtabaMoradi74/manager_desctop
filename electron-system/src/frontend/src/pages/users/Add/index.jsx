...
const Add = ({onClose}) => {
  ...
  const onSuccess = (response) => {
    ...
    const resetData = {
      ...params,
      role: {label: params?.role?.name, value: params?.role?.id},
      status: StatusData[params.status],
      type: Enum.enum.object[params?.type],
      image: {id: params?.image?.id, url: params?.image?.url, title: params?.image?.title},
    }
  ...
  const onSubmit = async () => {
    ...
    const reqData = {
      ...values,
      // [validation.fieldNames.username]: values[validation.fieldNames.username] || '',
      ...
      // [validation.fieldNames.isVerified]: !!values[validation.fieldNames.isVerified],
      [validation.fieldNames.role]: values[validation.fieldNames.role]?.value,
      ...
      [validation.fieldNames.status]: values[validation.fieldNames.status]?.value,
      // [validation.fieldNames.isFeatured]: !!values[validation.fieldNames.isFeatured],
      // [validation.fieldNames.approvedNft]: !!values[validation.fieldNames.approvedNft],
      // [validation.fieldNames.site]: values[validation.fieldNames.site],
      // [validation.fieldNames.instagram]: values[validation.fieldNames.instagram],
    }
              ...
              >
                <Grid container spacing={5}>
                  <Grid item xs={12} md={12}>
                    <RHFTextField
                      required
                      name={validation.fieldNames.firstName}
                      label={'firstName'}
                    />
                  </Grid>
                  <Grid item xs={12} md={12}>
                    <RHFTextField
                      required
                      name={validation.fieldNames.lastName}
                      label={'lastName'}
                    />
                  </Grid>
                  <Grid item xs={12} md={12}>
                    <RHFTextField
                      disabled={paramId}
                      name={validation.fieldNames.email}
                      label={'email'}
                    />
                  </Grid>
                  <Grid item xs={12} md={12}>
                    <RHFTextField
                      disabled={paramId}
                      name={validation.fieldNames.mobileNumber}
                      label={'mobileNumber'}
                    />
                  </Grid>
                  <Grid item xs={12} md={12}>
                  ...
                  <Grid item xs={12} md={12}>
                    <RHFTextField
                      required
                      type={'password'}
                      name={validation.fieldNames.confirmPassword}
              ...
              <FormItem label={t('select')}>
                <Grid container spacing={4}>
                  <Grid item xs={12} md={12}>
                    <RHFSelector
                      required
                      options={Enum.enum.array}
                      name={validation.fieldNames.type}
                      label={'type'}
                    />
                  </Grid>

                  {watchType?.value === 'ADMIN' ? (
                    <Grid item xs={12} md={12}>
                      <RoleSelector.Element
                        require
                        name={validation.fieldNames.role}
                        label='role'
                      />
                    </Grid>
                  ) : (
